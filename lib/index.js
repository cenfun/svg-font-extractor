...
const generateSvg = (options, item) => {
    ...
    //may generated by self handler onSVGItem
    if (!item.svg) {
        const {
            viewBox, fill, d
        } = item;
    ...
    if (!item.name) {
        item.name = item['glyph-name'] || item.index;
    }

    const output = options.output;
    if (output) {
        fs.writeFileSync(path.resolve(output, `${item.name}.svg`), item.svg);
    }

    options.count += 1;
    options.icons.push(item);
